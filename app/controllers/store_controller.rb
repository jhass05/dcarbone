class StoreController < ApplicationController
  ...
  before_filter :redirect_if_order_empty,
    :only => [
      :checkout, :select_shippisng_method, :view_shipping_method, :set_shipping_method,
  ...
  def close_sale
     ...
     while(@@pedidos.empty? == false)  
        @sale = Sale.new
        @pedido = @@pedidos.pop     
        @sale.product = @pedido['product']
        @sale.price   = @pedido['price']
        @sale.save
     end
